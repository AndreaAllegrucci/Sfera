...
int main( int argc, char* argv[] ) {
  ...
  if( argc != 3 && argc != 2) {
    ...
    std::cout << "USAGE1: ./asciiToTree [filename] \n USAGE2: ./asciiToTree [filename] [percorso (tipo date/canale_ch/)]" << std::endl;
    exit(1);
  ...
  std::string complete_fileName = path + fileName ;


   if( boost::starts_with(argv[1], "/run/media/andrea/Elements/") ) {   //se la stringa argv[1] inizia con ../data/ elimina ../data/ (tramite il metodo erase della classe string)
    fileName.erase( 0, 8 );
  }
  ...
  if( !fs.good() ) {
    std::cout << "-> No file called '" << fileName << "' found in '/run/media/andrea/Elements/'. Exiting." << std::endl;
    exit(1);
  }
  ...
  //per salvare in una cartella a parte
  
  /* std::string plotsDir(Form("tree/%s", path.c_str()));  
  ...
  if((pos = fileName.find(".")) != std::string::npos) {
    ...
    outfileName = plotsDir + prefix + ".root"; //ammessa somma tra stringhe
  } */
